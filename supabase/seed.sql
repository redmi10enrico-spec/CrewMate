...
insert into public.server_modes (name, slug, description, icon, "order", enabled) values
  ('Survival', 'survival', 'Sopravvivi, costruisci ed esplora un mondo persistente con economia e protezioni.', '⛏️', 1, true),
  ('SkyBlock', 'skyblock', 'Parti da un''isola nel vuoto e crea il tuo impero passo dopo passo.', '🏝️', 2, true),
  ('Minigames', 'minigames', 'Sfida gli altri giocatori in tante modalità competitive e frenetiche.', '⚔️', 3, true)
on conflict (slug) do nothing;

insert into public.home_features (title, description, icon, "order", enabled) values
  ('Anti-Cheat', 'Protezione avanzata contro cheater e griefer.', '🛡️', 1, true),
  ('Community', 'Staff attivo e giocatori accoglienti.', '🤝', 2, true),
  ('Performance', 'Hardware potente per zero lag.', '⚡', 3, true),
  ('Eventi', 'Eventi settimanali con premi esclusivi.', '🎁', 4, true)
